...
function initializeCaseBCalculator() {
    ...
    function calculateCaseBTarget() {
        ...
        // Fix 1: Round the target score to nearest integer
        let netTargetScore = Math.round(par1 + targetScoreforRemainingOvers);
        
        // Fix 2: Add error handling and type conversion
        try {
            if (isNaN(netTargetScore)) {
                updatedTargetInput.value = '';
                console.error('Invalid target score calculation');
                return;
            }
            updatedTargetInput.value = netTargetScore.toString();
            
            // Fix 3: Add debug logging
            console.log('Updated target:', {
                par1,
                targetScoreforRemainingOvers,
                netTargetScore
            });
        } catch (error) {
